...
consulta indice armarios = 
    case Map.lookup indice armarios of
        Nothing -> Nothing
        Just (estado, codigo) -> if estado == Alugado then Just Alugado else Just Livre
...
pegarCodigo indice armarios = 
    case pegarArmario indice armarios of
        Left x -> Left "nsei oq por"
        Right (estado, codigo) -> if estado == Alugado then Left "Ja esta ocupado" else Right codigo
...
devolucao indice codigo armarios = case pegarArmario indice armarios of
    Left aux -> Left $ aux
    Right (estado, cdx) -> if codigo == cdx
                            then if estado /= Livre then Right (Map.insert indice (Livre, cdx) armarios) else Left "Armario nao esta alugado"
